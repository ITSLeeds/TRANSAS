...
postcodes <- read.csv("data/postcode_lookup_W4.csv")
...
write.csv(postcodes, "data/postcode_lookup_with_data_w4.csv", row.names = FALSE)

# Get transaas postcodes
transas <- read.csv("data/W4HomeMovers_PCs.csv")
transas$postcode_clean <- toupper(gsub(" ","",transas$postcode_final_W4))
transas <- transas[,c("RecordNo","Location","NewHouse_W4","postcode_final_W4","postcode_clean")]

transas <- left_join(transas, postcodes, by = c("postcode_clean"))

write.csv(transas, "data/transas_id_with_data_w4.csv", row.names = FALSE)
...
postcodes <- readRDS(paste0(substr(secure_path,1,39),"Postcodes/code_point_open_2022.Rds"))
...
write.csv(transas_new, "data/transas_id_with_data_plusscotland_W4.csv", row.names = FALSE)
